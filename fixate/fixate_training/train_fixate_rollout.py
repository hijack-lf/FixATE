#!/usr/bin/env python3
"""Attention Rollout: data split, K-fold CV, grid search, final train/test."""
...
from fixate.probing_operator.rollout_attention import (
    EXPANDED_DIRS,
    RolloutAttentionMixin, PromptBasisModule, GazeDataset,
    ...
    _load_image, compute_rollout_freegeneration_with_soft_prompt,
    compute_baseline_rollout_freegeneration, compute_baseline_rollout_attention,
    compute_option_logprobs,
)


class RolloutTrainer(RolloutAttentionMixin):
    ...
    def __init__(self, model_name=MODEL_NAME, model_type=MODEL_TYPE, prompt_basis=None):
        ...
        self.eps = 1e-8; self.global_step = 0
    ...
    def _rollout_single_sample(self, i, imgs, pts, sps, cs, ama, choice_losses):
        """Process one sample: forward → rollout → slot attention."""
        ...
        with torch.enable_grad():
            ...
            if not attns:
                ama.append(sp.sum()*0.0 + torch.ones(NUM_SLOTS,device=sp.device,dtype=sp.dtype)/NUM_SLOTS)
                return
            # Differentiable rollout
            rollout = self._compute_rollout([a[0:1] for a in attns])
            if rollout is None:
                ama.append(sp.sum()*0.0 + torch.ones(NUM_SLOTS,device=sp.device,dtype=sp.dtype)/NUM_SLOTS)
                return
            # Query position from answer tokens
            ap = meta["answer_pos_combined"]
            if ap and len(ap) > 0:
                qps = [p-1 for p in ap if p-1 >= 0]
            else: qps = []
            if not qps: qps = [rollout.shape[0]-1]
            acc = None
            for qp in qps:
                qp = max(0, min(qp, rollout.shape[0]-1))
                v = rollout[qp]
                acc = v if acc is None else acc + v
            vis = self.find_visual_token_indices(meta["orig_input_ids"], meta["soft_len"])
            if acc is None or not vis:
                ama.append(sp.sum()*0.0 + torch.ones(NUM_SLOTS,device=sp.device,dtype=sp.dtype)/NUM_SLOTS)
                return
            kv_len = acc.shape[0]; vis = [v for v in vis if 0<=v<kv_len]
            if not vis:
                ama.append(sp.sum()*0.0 + torch.ones(NUM_SLOTS,device=sp.device,dtype=sp.dtype)/NUM_SLOTS)
                return
            ma = self._rollout_to_slot_attention(rollout, qps[-1], vis, meta, img)
            ama.append(ma)
    ...
    def train_step(self, batch, optimizer, accumulation_steps=1, zero_grad=True, do_optimizer_step=True):
        ...
        for i in range(bs): self._rollout_single_sample(i, imgs, pts, sps, cs, ama, cl)
    ...
    def train_step_batched(self, batch, optimizer, accumulation_steps=1, zero_grad=True, do_optimizer_step=True):
        ...
        with torch.enable_grad():
            ...
            ama = []
            for i in range(bs):
                m = per_meta[i]; sp = sps[i].to(ed); img = imgs[i]
                ap = m["answer_pos_combined"]
                qps = [p-1 for p in ap if p-1>=0] if ap else []
                if not qps: qps = [slens[i]-1]
                rollout = None
                for l_idx in range(L):
                    attn_avg = attns[l_idx][i].mean(dim=0).to(torch.float32)
                    N = attn_avg.shape[0]; ah = 0.5*attn_avg + 0.5*torch.eye(N,device=attn_avg.device,dtype=attn_avg.dtype)
                    ah = ah/(ah.sum(-1,keepdim=True)+1e-12)
                    rollout = ah if rollout is None else ah @ rollout
                if rollout is None:
                    ama.append(sp.sum()*0.0+torch.ones(NUM_SLOTS,device=sp.device,dtype=sp.dtype)/NUM_SLOTS); continue
                acc = None
                for qp in qps:
                    qp = max(0,min(qp,rollout.shape[0]-1)); v = rollout[qp]
                    acc = v if acc is None else acc+v
                vis = self.find_visual_token_indices(m["orig_input_ids"], m["soft_len"])
                if acc is None or not vis:
                    ama.append(sp.sum()*0.0+torch.ones(NUM_SLOTS,device=sp.device,dtype=sp.dtype)/NUM_SLOTS); continue
                kv = acc.shape[0]; vis = [v for v in vis if 0<=v<kv]
                if not vis:
                    ama.append(sp.sum()*0.0+torch.ones(NUM_SLOTS,device=sp.device,dtype=sp.dtype)/NUM_SLOTS); continue
                ma = self._rollout_to_slot_attention(rollout, qps[-1], vis, m, img)
                ama.append(ma)
        ab = torch.stack(ama); gd = gd.to(ab.device)
        law = self.config.lambda_attn_weight
        total, ld = self.compute_losses(ab, gd, uids, law, self.config.beta_reg, loss_choice=loss_choice)
...
def evaluate_freegen(trainer, samples, uf, pd, ed=None, use_soft_prompt=True):
    ...
    for s in samples:
        ...
        try:
            if use_soft_prompt: attn, gen = compute_rollout_freegeneration_with_soft_prompt(trainer, uid, img, pt)
            else: attn, gen = compute_baseline_rollout_freegeneration(trainer, img, pt)
        except Exception:
            ns += 1
            continue
...
def evaluate_tf(trainer, samples, uf, pd, ed=None, use_soft_prompt=True):
    ...
    for s in samples:
        ...
        try:
            if use_soft_prompt: ma = trainer.compute_model_attention_single(uid, img, pt, at)
            else: ma = compute_baseline_rollout_attention(trainer, img, pt, at)
...
# ═══════════════════════════════════════════════════════════════
#  Pipeline (identical structure to attnlrp)
# ═══════════════════════════════════════════════════════════════
...
def _split(samples, K=5, seed=42):
...
def train_one_fold(trainer, ts, vs, uf, pd, ed, hp, ne, ee):
    apply_hyperparams(trainer, hp); reset_prompt_basis(trainer, hp)
    for uid in set(s["user_id"] for s in ts+vs): trainer.prompt_basis.get_or_create_user_alpha(uid)
    opt = build_optimizer(trainer, hp)
    loader = DataLoader(GazeDataset(ts, uf, pd, expanded_dirs=ed), batch_size=BATCH_SIZE, shuffle=True, num_workers=0, collate_fn=collate_fn)
    ...
    for epoch in range(1, ne+1):
        ...
        if (epoch%ee==0) or epoch==ne:
            if torch.cuda.is_available(): torch.cuda.empty_cache()
            vm = evaluate_tf(trainer, vs, uf, pd, ed)
...
def grid_search_cv(trainer, samples, folds, uf, pd, ed, param_grid, ne, ee):
    ...
    with open(os.path.join(OUTPUT_DIR, f"rollout_grid_{RUN_TIMESTAMP}.json"), "w") as f: json.dump({"best_hp": best_hp, "best_score": best_sc}, f, indent=2)
...
def final_train_and_test(trainer, all_samples, test_idx, uf, pd, ed, hp, ne, val_ratio=0.0):
    ...
    trainer.save_checkpoint(os.path.join(CHECKPOINT_DIR, f"rollout_final_{RUN_TIMESTAMP}.pt"))
    ...
    with open(os.path.join(OUTPUT_DIR, f"att_rollout_final_{RUN_TIMESTAMP}.json"), "w") as f:
...
def main():
    ...
    test_idx, folds = _split(samples, K_FOLDS, SEED)
    trainer = RolloutTrainer(MODEL_NAME, MODEL_TYPE)
    if RUN_GRID_SEARCH:
